"""
timeline.py - יצירת ציר זמן אינטראקטיבי
צוות 1, זוג B
"""
...
def generate_camera_dashboard(raw_data):
    ...
    for idx, row in grouped.iterrows():
        ...
        filenames_list = list(row['filenames'])
        ...
        if display_name not in device_colors:
            ...
            # === העיצוב המקורי והחצי-שקוף עם ההילה (Glow) חזר ===
            dynamic_css += f"""
            /* צבע השורה */
            .vis-group.{group_class} {{ 
                background-color: {hex_to_rgba(color, 0.04)} !important; 
                border-bottom: 1px solid rgba(255,255,255,0.03) !important; 
            }}
            .vis-label.{group_class} {{ 
            ...
            /* אנימציית נשימה/הבהוב מותאמת אישית לנקודות ולאשכולות */
            ...
            /* עיצוב האגד (Cluster) - הוא מכיל את המספרים ונראה כמו בועה שקופה */
    ...
    <style>
        ...
        .vis-group {{ 
            background-image: linear-gradient(to bottom, transparent 49%, rgba(255, 255, 255, 0.08) 50%, rgba(255, 255, 255, 0.08) 51%, transparent 52%); 
        }}
        ...
        /* אין חלון קופץ מרחף - פותחים הכל בלחיצה! */
    ...
    <script>
        ...
        var options = {{
            ...
            // החזרנו את האשכולות כדי לראות את המספרים בתוך הבועות!
            cluster: {{
...
if __name__ == "__main__":
    ...
    success, timeline_html = generate_camera_dashboard(test_data)
    if success:
